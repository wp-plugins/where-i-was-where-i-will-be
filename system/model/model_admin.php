...
*/    
    ...
    function add_type($info) {
        return $this->db->query('INSERT INTO '.WIW_TABLE_PREFIX.'type (id,name,pin) 
                                VALUES (NULL,"'.$info['type_name'].'","'.$info['type_pin'].'")');
    }
    
    function edit_type($info) {
        if ($info['type_id'] != 1) {  //Don't allow to change the first row
            return $this->db->query('UPDATE '.WIW_TABLE_PREFIX.'type SET name = "'.$info['type_name'].'",pin = "'.$info['type_pin'].'" 
                                WHERE id = "'.$info['type_id'].'"');
        } else return false;
    }
